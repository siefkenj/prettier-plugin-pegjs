...
Grammar
  = __ initializer:(a:Initializer {return a})? __? rules:(a:Rule __ {return a})+ {

        return createNode("grammar", {initializer, rules, comments, location:location() });

    }

Initializer
  = code:(code:CodeBlock {return createNode( "initializer", { code } );}) EOS {
      return code
...
Rule
  = name:Identifier
    __
    displayName:(
      a:(a:StringLiteral { return createNode("stringliteral", { value: a }); })
      __ { return a; }
    )?
    delimiter:("=" { return createNode("delimiter", { value: "=" }); })
    __
    expression:Expression
    EOS { return createNode("rule", { name, displayName, expression, delimiter }); }
...
ChoiceExpression
  = head:ActionExpression tail:(__ 
      delimiter:("/" { return createNode("delimiter", { value: "/" }); })
   __ a:ActionExpression {return [a, delimiter]})* {

        if ( tail.length === 0 ) {return head};

        return createNode( "choice", {
            alternatives: [ head ].concat( tail.map(x => x[0]) ),
            delimiters: tail.map(x => x[1])
        } );

    }

ActionExpression
  = expression:SequenceExpression code:(__ a:CodeBlock {return a})? {
...
SequenceExpression
  = head:LabeledExpression tail:(__ a:LabeledExpression {return a})* {
...
Comment "comment"
  = a:MultiLineComment { return createNode("comment", {block: true, text: a}) }
  / a:SingleLineComment { return createNode("comment", {block: true, text: a}) }
...
IdentifierStart
  ...
  / "$"
  / "_"
  / "\\" a:UnicodeEscapeSequence {return a}
...
DoubleStringCharacter
  = !('"' / "\\" / LineTerminator) a:SourceCharacter {return a}
  / "\\" a:EscapeSequence {return "\\"+a}
  / LineContinuation
...
ClassCharacter
  = !("]" / "\\" / LineTerminator) a:SourceCharacter {return a}
  // This line needs to be changed from the actual `pegjs.pegjs` file
  // to produce consistent parser output. See https://github.com/pegjs/pegjs/issues/650
  / "\\" a:EscapeSequence { return  /*SPECAIL COMMENT*/"\\"+a }
  / LineContinuation
...
NonEscapeCharacter
  = !(EscapeCharacter / LineTerminator) a:SourceCharacter {return a}
