...
ChoiceExpression
  = head:ActionExpression
    tail:(
      __
        delimiter:("/" { return createNode("delimiter", { value: "/" }); })
        __
        a:ActionExpression { return [a, delimiter]; }
    )* {
      if (tail.length === 0) {
        return head;
      }
...
Comment "comment"
  = a:MultiLineComment {
      return createNode("comment", { block: true, text: a });
    }
  / a:SingleLineComment {
      return createNode("comment", { block: true, text: a });
    }
...
HexEscapeSequence = "x" digits:$(HexDigit HexDigit) { return "x" + digits; }

UnicodeEscapeSequence
  = "u" digits:$(HexDigit HexDigit HexDigit HexDigit) { return "u" + digits; }

DecimalDigit = [0-9]

HexDigit = [0-9a-f]i

AnyMatcher = "." { return createNode("any"); }
